...
- Capture DevTools network at launch and every feature path.
- Confirm fetch/XHR/WebSocket/EventSource/sendBeacon are blocked.
- Confirm no service worker is registered.
- Attribute any hosted platform requests separately.
...
- Inject payloads into pet names, BLE names, rescue packs, backups, emergency fields, diagnostics.
- Verify action dispatch blocks unapproved method calls and unsafe args.
- Verify Trusted Types behavior on Chromium and fallback behavior elsewhere.
...
- Test wrong passphrase, truncated packs, oversized files, deep objects, too many pets, hostile settings, hostile voice data.
- Verify encrypted-only default export and no plaintext secret leakage.
- Check merge/replace and partial failure paths.
...
- Populate all stores and test soft reset, hard reset, factory reset, auto-wipe, crash/reopen.
- Inspect IndexedDB, localStorage, Cache Storage, service worker registrations, object URLs, downloads, clipboard, browser history.
...
- Test registration consent and 30-second auto-stop.
- Test crowded BLE and malicious advertisement names.
- Assess scan restart timing, local radio detectability, focus mode, filters vs accept-all.


---


### ML/state/diagnostics

- Import adversarial ML state and verify reset recovery.
- Confirm diagnostics redaction and self-test behavior.
- Verify error logs are bounded and locally stored.
